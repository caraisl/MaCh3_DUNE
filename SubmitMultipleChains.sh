...
if [[ $HOSTNAME == *gpu.rl.ac.uk ]]; then
  ...
  IsPerlmutter=true
  echo "Found Perlmutter, setting scratch"
  ScratchDir="$SCRATCH"
  SEC_PER_STEP=0.6

elif [[ $HOSTNAME == *scarf.rl.ac.uk ]]; then
  IsScarf=true
  echo "Found SCARF, setting scratch"
  ScratchDir="/work4/ppd/scarf1583"
  SEC_PER_STEP=0.08
  RAM=1
...
##################
# Output directory
OutputDir=${ScratchDir}/"Results"/${FitName}
...
for ((i = 0 ; i < $NCHAINS ; i++)); do
  ...
  sed -i "s|^  OutputFile.*|  OutputFile: \"${OutputDir}/${JobName}.root\"|g" $cfgFileName
  ...
  echo "source setup_dune_perl_micromamba.sh" >> $ScriptFileName
  ...
  if [[ ${IsEmerald} == true ]]; then
    ...
    then
      ...
      PrevJobPID=$(squeue --me --format="%.18i %.9P %.60j %.8u %.8T %.10M %.9l %.6D %R" | grep "${OldJobName}" | awk '{print $1}')
      qsubopt="--partition=scarf --time=${WALLTIME_CC} --cpus-per-task=${NTHREADS} --mem=${RAMMB}M --dependency=afterany:${PrevJobPID}"
      echo "This job will wait for job with name: ${OldJobName} with PID: "${PrevJobPID}""
    else
      qsubopt="--partition=scarf --time=${WALLTIME_CC} --cpus-per-task=${NTHREADS} --mem=${RAMMB}M"
    fi
    # Also different way of specifiying output
    stdout="--output ${logDir}/${JobName}_%j.log"
    stderr="--error ${logDir}/${JobName}_%j.err"
